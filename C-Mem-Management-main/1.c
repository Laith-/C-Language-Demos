...
int *subArrPointer(const int anArray[], int arrLen, int position, int size) {
  int subSize = position + size;
  if (position < 0 || position >= arrLen || subSize > arrLen)
  {
    return NULL;
  }
  
  size_t memSize = sizeof(int) * size;
  int * subArray = malloc(memSize);
  if (subArray == NULL)
  {
    return NULL;
  }
...
int* insert(const int anArray[], int arrLen, int position, int value) {
  ...
  int newLen = arrLen + 1;
  size_t memSize = sizeof(int) * newLen;
  int *newArray = malloc(memSize);
  if (newArray == NULL)
  {
    return NULL;
  }
  ...
  for (int i = position + 1; i <= newLen; i++)
    {
...
int* erase(const int anArray[], int arrLen, int position) {
  ...
  int newLen = arrLen - 1;
  int *erasedArray = malloc(sizeof(int) * newLen);
